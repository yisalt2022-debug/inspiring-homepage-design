...
export function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <button
      ...
      className={`group relative inline-flex shrink-0 items-center gap-0.5 rounded-md border border-primary/40 bg-card/40 backdrop-blur-sm p-0.5 transition-all hover:border-primary/70 hover:shadow-[0_0_18px_color-mix(in_oklab,var(--primary)_45%,transparent)] ${
        compact ? "h-9" : "h-9 md:h-10"
      }`}
    >
      <span
        className={`flex h-6 w-6 md:h-7 md:w-7 items-center justify-center rounded transition-all ${
          !isCyber
